...
The runnable DailyMart live adapter path uses `scripts/maws_transfer_bridge.mjs` as an HTTP-to-MCP bridge because iOS cannot speak stdio MCP directly. DailyMart posts `schema_version=meshkit-maws-transfer-send-bridge/v1`, `tool=transfer.send`, M-AWS arguments, and the full provider-neutral MeshKit OKRW execution transaction request. The bridge calls M-AWS MCP `transfer.send`, then queries maroo JSON-RPC `eth_getTransactionReceipt` to return `txHash`, `blockHash`, `blockNumber`, `confirmationCount`, and `confirmedAt`. DailyMart must treat a bridge response that has `txHash` but lacks receipt proof as `pending`, not `confirmed`.

Live demo readiness is recorded by `python3 scripts/verify_maws_live_readiness.py`, which writes `artifacts/maroo-testnet/maws-live-readiness.json`. Missing `MESHKIT_MAWS_BRIDGE_URL`, `MESHKIT_MAWS_AGENT_ID`, `WAAS_AUTH_TOKEN` or stored M-AWS credentials is `BlockedByExternalChain` evidence with `funded_wallet_unavailable` or `payment_confirmation_unavailable`; this evidence keeps HermesChat delegated remaining limit unchanged and prevents any deterministic fallback from being displayed as paid or complete.

Bridge behavior is contract-tested without credentials by `node scripts/verify_maws_transfer_bridge_contract.mjs`. The test uses local fake M-AWS MCP and fake maroo RPC servers to prove the bridge maps a real-style `transfer.send` tx hash plus `eth_getTransactionReceipt` into confirmed proof fields, and that `POLICY_REJECTED` remains a no-transaction policy denial. The fake MCP speaks stdio MCP `Content-Length` framing so the bridge contract matches the real server transport rather than a line-delimited JSON shortcut. This contract test is necessary but not sufficient for live demo completion; final proof still requires a real M-AWS agent wallet, funded OKRW balance, physical iPad DailyMart execution, and explorer-verifiable maroo transaction receipt.

The installed M-AWS MCP surface is probed independently by `node scripts/probe_maws_mcp_stdio.mjs`. It writes `artifacts/maroo-testnet/maws-mcp-stdio-probe.json` and passes only when the configured `m-aws serve` command responds to `initialize`/`tools/list` and exposes `transfer.send`. If the npm package starts but exits before MCP initialization, the readiness evidence must classify that as `maws_mcp_unavailable` instead of blaming maroo RPC or fabricating a payment state.

For a command-line live proof before or alongside the iPad demo, `node scripts/probe_maws_live_transfer.mjs` posts to the configured bridge and writes `artifacts/maroo-testnet/maws-live-transfer-proof.json`. The proof artifact must contain `confirmed=true`, `txHash`, `blockHash`, `blockNumber`, `confirmationCount`, `confirmedAt`, and `explorerUrl` before a DailyMart OKRW transfer can be represented as paid or complete. Any missing field records `BlockedByExternalChain` instead.

When M-AWS is unavailable, DailyMart can use the direct maroo testnet transfer adapter path through `scripts/maroo_native_okrw_transfer_bridge.mjs`. DailyMart selects this bridge when `MESHKIT_MAROO_OKRW_TRANSFER_BRIDGE_URL` is present, posts `schema_version=meshkit-maroo-native-okrw-transfer-bridge/v1`, `tool=maroo.native_transfer`, the EVM recipient, OKRW amount, idempotency token, and the full MeshKit transaction request, and receives the same provider-neutral proof fields. The bridge is testnet-only, uses `MESHKIT_MAROO_PRIVATE_KEY`, signs a native OKRW transaction through maroo JSON-RPC, waits for the receipt, and must never return confirmed without `txHash`, `blockHash`, `blockNumber`, `confirmationCount`, `confirmedAt`, and `explorerUrl`.

The direct maroo proof probe is `node scripts/probe_maroo_native_okrw_transfer.mjs`, which writes `artifacts/maroo-testnet/maroo-native-okrw-transfer-proof.json`. It is the authoritative command-line proof for the non-M-AWS route. DailyMart's default delegated recipient is an EVM address so the M-AWS and direct maroo clients can execute against the same policy-bound recipient instead of a placeholder logical merchant id.

Direct bridge readiness is recorded by `python3 scripts/verify_maroo_native_okrw_readiness.py`, which writes `artifacts/maroo-testnet/maroo-native-okrw-readiness.json`. Missing `MESHKIT_MAROO_OKRW_TRANSFER_BRIDGE_URL`, missing `MESHKIT_MAROO_PRIVATE_KEY`, or a failed bridge health check is `BlockedByExternalChain` evidence and cannot be presented as payment completion.

Direct bridge behavior is contract-tested without a private key by `node scripts/verify_maroo_native_okrw_transfer_bridge_contract.mjs`. The test injects a fake ethers provider through `MESHKIT_MAROO_ETHERS_MODULE` to prove confirmed, pending, failed, and invalid-recipient cases without broadcasting a transaction. This proves the bridge contract and proof mapping, not live settlement; live settlement still requires the direct proof artifact with maroo explorer-verifiable receipt fields.

Aggregate demo readiness is recorded by `python3 scripts/verify_maroo_demo_readiness.py`, which writes `artifacts/maroo-testnet/demo-readiness.json`. The aggregate separates `deterministicReady` from `liveConfirmed`: deterministic readiness can pass when contract tests, Swift tests, docs verification, public maroo endpoint checks, faucet availability/manual-funding readiness, the DailyMart iPad simulator build, and the DailyMart pending proof UI test pass, but `liveConfirmed` remains false until either M-AWS or direct maroo proof artifacts contain explorer-verifiable confirmed receipt fields. `MESHKIT_IOS_SIMULATOR_DESTINATION` can override the default iPad simulator destination for local machines with different simulator runtimes.

The maroo faucet is modeled as a wallet-connected manual funding dependency, not an automatic SDK side effect. `python3 scripts/verify_maroo_faucet_readiness.py` records availability for https://faucet.maroo.io/ and writes `artifacts/maroo-testnet/faucet-readiness.json` with the manual funding steps; confirmed payment readiness still requires a later txHash/block/explorer proof from M-AWS or the direct maroo bridge.

For hackathon demos, the recommended minimum live path is the direct maroo testnet bridge with a newly generated, testnet-only EVM wallet. `node scripts/create_maroo_demo_wallet.mjs` writes an ignored `.env.maroo-demo.local` file with `MESHKIT_MAROO_PRIVATE_KEY`, `MESHKIT_MAROO_WALLET_ADDRESS`, `MESHKIT_MAROO_FAUCET_WALLET_ADDRESS`, and probe defaults, while `artifacts/maroo-testnet/demo-wallet.json` records only non-secret setup metadata. Wallet creation alone does not create testnet balance: the operator must import the generated testnet-only key into a fresh MetaMask account, or use a fresh MetaMask test account and copy that test-only private key into the env file, then request faucet funds for the same sender address. After funding, the operator starts `scripts/maroo_native_okrw_transfer_bridge.mjs`, runs `scripts/probe_maroo_native_okrw_transfer.mjs`, then re-runs aggregate readiness. The private key remains a local testnet secret and must never be copied into source, docs, screenshots, or production configuration.

`node scripts/verify_maroo_demo_wallet_setup.mjs` verifies the wallet setup before funding or bridge startup. It derives the signer address from the local private key without printing the secret, requires it to match `MESHKIT_MAROO_WALLET_ADDRESS` and `MESHKIT_MAROO_FAUCET_WALLET_ADDRESS`, and writes `artifacts/maroo-testnet/demo-wallet-setup.json`. This catches the most common demo failure mode: funding one MetaMask account while the bridge signs with a different private key.

For physical iPad runs, the DailyMart app cannot reach a bridge URL on Mac loopback (`127.0.0.1` or `localhost`). `scripts/install_ios_device.sh` loads `.env.maroo-demo.local`, rewrites loopback M-AWS or direct maroo bridge URLs to `MESHKIT_IOS_BRIDGE_HOST` for launch, and records a redacted `DailyMart-launch-environment.json`. This keeps the signing private key local to the Mac-side bridge while the iPad receives only the reachable bridge endpoint and optional redacted authorization proof.

`bash scripts/verify_ios_device_maroo_launch_env.sh` fakes Xcode/devicectl and verifies the physical-device launch contract deterministically: loopback bridge URLs are rewritten to a Mac LAN host, authorization values are redacted in artifacts, and `MESHKIT_MAROO_PRIVATE_KEY` is not passed into the iPad launch environment.

The Mac-side direct bridge must also bind to an address reachable from the iPad. Set `MESHKIT_MAROO_OKRW_TRANSFER_BRIDGE_HOST=0.0.0.0` when starting `scripts/maroo_native_okrw_transfer_bridge.mjs`, or use `scripts/run_maroo_native_okrw_live_proof.mjs` with `MESHKIT_IOS_BRIDGE_HOST` so the guarded runner widens the bind host automatically. Binding remains a local demo operator choice; the private key stays on the Mac and is never launched inside DailyMart.

When the direct bridge is reachable off-machine, `MESHKIT_MAROO_OKRW_TRANSFER_AUTHORIZATION` must be shared only as a launch-time header between DailyMart/probes and the Mac-side bridge. The bridge requires that exact header for POST `/transfer` when configured, while `/health` remains non-secret readiness evidence. `scripts/create_maroo_demo_wallet.mjs` generates an authorization header for new demo wallets.

The direct bridge enforces optional server-side spend policy before signing: `MESHKIT_MAROO_OKRW_ALLOWED_RECIPIENTS` restricts recipient addresses and `MESHKIT_MAROO_OKRW_TRANSFER_MAX_AMOUNT` caps per-transfer OKRW amount. `scripts/create_maroo_demo_wallet.mjs` writes defaults for new demo wallets, and `scripts/ensure_maroo_bridge_policy.mjs` adds the same defaults to an existing ignored env file without changing its wallet address.

`node scripts/maroo_demo_operator_status.mjs` writes a non-secret operator summary to `artifacts/maroo-testnet/demo-operator-status.json` and `.md`. It reads current readiness/proof artifacts and maroo RPC balance, then reports signer address, faucet status, bridge policy state, proof state, and the next command without printing the private key or bridge authorization token.
...
- Direct maroo Agent Wallet Kit integration remains a future replaceable adapter path; the MVP adapter may use M-AWS, direct native maroo testnet transfer, deterministic request construction, RPC probing, or configured live transaction references while preserving the same provider-neutral inputs and outputs.
